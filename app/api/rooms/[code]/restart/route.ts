...
export async function POST(
  ...
  try {
    ...
    const { boardType = 'custom', playerId } = await request.json();
    ...
    if (playerError || !player || !player.is_host) {
      return NextResponse.json(
        { success: false, error: 'error.hostOnly' },
        { status: 403 }
      );
    }
    ...
    const logMessage = tWithParams('gameLog.gameRestarted', { board: boardName, count: players.length }, lang);
